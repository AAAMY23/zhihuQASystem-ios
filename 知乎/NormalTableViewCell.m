...
@property(nonatomic,strong,readwrite) UILabel *readNumber;
@property(nonatomic,strong,readwrite) UILabel *comment;
@property(nonatomic,strong,readwrite) UIImageView *image1;
@property(nonatomic,strong,readwrite) UIImageView *answerCreatorPhoto;
@end
...
- (instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(nullable NSString *)reuseIdentifier{
    ...
    if(self){
        ...
        [self.contentView addSubview:({
            self.user=[[UILabel alloc]initWithFrame:CGRectMake(40.f, 80.f, 120.f, 15.f)];
        ...
        [self.contentView addSubview:({
            self.answerCreatorPhoto=[[UIImageView alloc]initWithFrame:CGRectMake(15.f, 80.f, 15.f, 15.f)];
            self.answerCreatorPhoto;
        })];
        [self.contentView addSubview:({
            self.answer=[[UILabel alloc]initWithFrame:CGRectMake(15.f, 80.f, 350.f, 90.f)];
        ...
        [self.contentView addSubview:({
            self.readNumber=[[UILabel alloc]initWithFrame:CGRectMake(15.f, 140.f, 70.f, 50.f)];
            //self.good.text=@"1.9w赞同";
            //[self.good sizeToFit];自动调节宽度 后面写
            self.readNumber.font=[UIFont systemFontOfSize:15.f];
            self.readNumber.textColor=[UIColor colorWithRed:153/255.0 green:153/255.0 blue:153/255.0 alpha:1.0];
            self.readNumber;
        })];
         
        [self.contentView addSubview:({
            self.comment=[[UILabel alloc]initWithFrame:CGRectMake(85.f, 140.f, 70.f, 50.f)];
...
-(void)layoutTableviewCellwithItem:(questionPlazaLoaditem *)listArray{
    ...
    self.readNumber.text=[NSString stringWithFormat:@"%@浏览",listArray.viewCount];
    //self.good.text=@"1.9w赞同";
    self.comment.text=[NSString stringWithFormat:@"%@评论",listArray.answerCount];
    //self.comment.text=@"585评论";
    //self.image1.image=[UIImage imageNamed:@"sucai2.jpg"];
    //self.answerCreatorPhoto.backgroundColor=[UIColor purpleColor];
    [self.answerCreatorPhoto sd_setImageWithURL:[NSURL URLWithString:listArray.answerCAvatarUrl]];
    }
